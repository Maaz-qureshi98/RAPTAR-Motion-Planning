...
def create_properly_oriented_l_shape(end_effector_link):
    ...
    roll = 0
    pitch = 0
    yaw = 140  # Rotation as per user specification was set to 150
    q = quaternion_from_euler(roll, pitch, yaw)

    # Define Box1: Horizontal part of the L (Shorter Side)
    box1 = SolidPrimitive()
    box1.type = SolidPrimitive.BOX
    box1.dimensions = [0.09, 0.05, 0.02]  # [length (x), width (y), height (z)] in meters

    box1_pose = Pose()
    box1_pose.orientation = Quaternion(*q)  # Apply rotation
    box1_pose.position.x = 0.005 # Half of box1's length to extend from the flange it was 0.01, -0.01, 0.01
    box1_pose.position.y = -0.02
    box1_pose.position.z = 0.01

    # Define Box2: Vertical part of the L (Longer Side)
    box2 = SolidPrimitive()
    box2.type = SolidPrimitive.BOX
    box2.dimensions = [0.02, 0.08, 0.1]  # [length (x), width (y), height (z)] in meters

    box2_pose = Pose()
    box2_pose.orientation = Quaternion(*q)  # Apply the same rotation it was 0.05, -0.05, 0.02
    box2_pose.position.x = 0.014  # Positioned exactly at the end of Box1
    box2_pose.position.y = -0.072
    box2_pose.position.z = -0.03  # Half of box2's height to stand upright
...
def main():
    ...
    scene.attach_box(end_effector_link, l_shape.id, touch_links=["panda_link8"])
    rospy.sleep(1)
