...
def get_A_T_G(phi, theta):
    ...
    """
    radius = 0.09  # Radius of the hemisphere (9 cm or change as per goal)
...
def main():
    ...
    O_T_A[0, 3] = 0.5 #current_pose.position.x
    O_T_A[1, 3] = 0.1 #current_pose.position.y
    O_T_A[2, 3] = 0.1  # Center point as in your original code
    ...
    for phi in phi_values:
        ...
        for theta in theta_values:
            ...
            phi_deg = degrees(phi)
            print(f"  Moving to phi = {phi_deg:.1f} deg, theta = {theta_deg:.1f} deg")
           ...
           # print(f"    O_T_FG:\n{O_T_FG}")
            ...
            if delta0 < deltapi:
                selected_plan = plan0
                #print("    Selected Plan: wpose0 (0° rotation)")
            else:
                selected_plan = planpi
                #print("    Selected Plan: wposepi (180° rotation)")

            # Execute the selected plan if valid
            if selected_plan:
                move_group.execute(selected_plan, wait=True)
                move_group.stop()
            ...
            # Wait before moving to the next pose
            time.sleep(1.0)
